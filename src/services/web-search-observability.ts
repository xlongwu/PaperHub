import { getWebSearchCacheCounts, getWebSearchMaintenanceState } from "@/db/web-search";
import { listWebSearchConnections } from "@/db/web-search-connections";
import { getWebSearchMetrics, type WebSearchMetricsReport } from "@/db/web-search-metrics";
...
export function getWebSearchMetricsForRange(from?: string, to?: string): WebSearchMetricsReport {
  const end = to ? parseDate(to, "to") : new Date();
  const start = from ? parseDate(from, "from") : new Date(end.getTime() - 7 * 24 * 60 * 60 * 1000);
...
export function getWebSearchHealth(): WebSearchHealth {
  ...
  const providers = ["arxiv", "openalex", "crossref", "tavily", "brave", "github", "mcp"].map(
    (providerId) => {
  ...
  const providerFailureRate = providerCalls > 0 ? 1 - successfulCalls / providerCalls : 0;
  return {
    status:
    ...
    lastSevenDays: {
      ...
      providerFailureRate,
      estimatedCredits: metrics.providers.reduce((sum, provider) => sum + provider.estimatedCredits, 0),
    },
  };
}
