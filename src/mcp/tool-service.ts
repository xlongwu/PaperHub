...
const MCP_PROVIDER_WHITELIST = new Set(["arxiv", "openalex", "crossref", "tavily", "brave", "github", "mcp"]);
...
export async function searchLocalLibraryTool(input: Record<string, unknown>) {
  ...
  return hybridSearch({
    query,
    mode: input.mode === "keyword" || input.mode === "semantic" ? input.mode : "hybrid",
    limit,
...
export function parseMcpWebSearchRequest(input: Record<string, unknown>): WebSearchRequest {
  const query = requiredString(input.query, "query", 500);
  const scope = input.scope === "technical" || input.scope === "mixed" ? input.scope : "academic";
  const searchBudget = parseSearchBudget(input.searchBudget);
  if (searchBudget === "broad" && process.env["PAPERHUB_MCP_ALLOW_BROAD_SEARCH"] !== "1") {
    throw new Error("Broad Search is disabled for MCP. Set PAPERHUB_MCP_ALLOW_BROAD_SEARCH=1 to opt in.");
  }
  ...
  const contentTypes = optionalStringArray(input.contentTypes, "contentTypes", 5);
  if (contentTypes?.some((contentType) => !MCP_CONTENT_TYPE_WHITELIST.has(contentType))) {
  ...
  return {
    ...
    providers,
    includeDomains: optionalStringArray(input.includeDomains, "includeDomains", 20),
    excludeDomains: optionalStringArray(input.excludeDomains, "excludeDomains", 20),
...
function parseOptionalDateRange(value: unknown): WebSearchRequest["dateRange"] {
  if (value === undefined) return undefined;
...
function parseLocalDateRange(value: unknown): { start: string; end: string } | undefined {
